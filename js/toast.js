...
(function($,window){ 
	...
	$.fn.toast = function(options){
		...
		return this.each(function(){
			...
			defaults.createMessage2 = function(){
				if(opt.closePrev){
					$('.cpt-toast').remove();
				}
				box = $("<span class='animated "+opt.animateIn+" cpt-toast'></span>").css({
					...
					'-moz-animation-duration':opt.animateDuration/1000+'s',
					'animation-duration':opt.animateDuration/1000+'s',
				}).html("<img style='width: 100%' src='"+opt.content+"'/>").appendTo($this);
				defaults.colseMessage();
			}
			...
			if(defaults.type=="img"){
				defaults.createMessage2();
			}else{
